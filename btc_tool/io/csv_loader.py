...
def _load_uphold_csv(
    ...
    Transaction type semantics:
      - "transfer": exchange/swap between Uphold cards (buy/sell/crypto-to-crypto)
      - "in":       incoming deposit from external wallet — buy lot (skipped if skip_uphold_in=True)
      - "out":      withdrawal to external wallet — NOT a taxable disposal; SpecialEvent only
      - "staking-reward" / "reward": staking income
    """
    trades: List[Trade] = []
    special_events: List[SpecialEvent] = []
    raw_rows: List[Dict] = []
    ...
    for row in reader:
        ...
        # ── "in" / staking-reward / reward ───────────────────────────────────
        if tx_type in {"in", "staking-reward", "reward"}:
            is_plain_deposit = tx_type == "in"

            if is_plain_deposit and skip_uphold_in:
                # Deposit already covered by source exchange CSV — skip to avoid duplicate lot
                skipped_asset = dest_asset if (dest_asset and not _is_fiat(dest_asset)) else origin_asset
                skipped_amount = dest_amount if dest_amount > 0 else origin_amount
                special_events.append(
                    SpecialEvent(
                        date=date,
                        asset=skipped_asset or "UNKNOWN",
            ...
            elif dest_asset and dest_amount > 0 and not _is_fiat(dest_asset):
                price = 0.0
                ...
                trades.append(Trade(date=date, asset=dest_asset, type="buy", amount=dest_amount, price=price, fee=0.0))

                if "reward" in tx_type or "staking" in tx_type:
                    special_events.append(SpecialEvent(date=date, asset=dest_asset, event_type=tx_type, amount=dest_amount, price=price, fee=0.0, notes=""))

        # ── "out": withdrawal to external wallet ─────────────────────────────
        # NOT a taxable disposal — record as SpecialEvent only, never as SELL
        elif tx_type == "out":
        ...
        # ── "transfer": internal Uphold card-to-card exchange ────────────────
            ...
            if origin_asset == dest_asset:
                # Same-asset internal transfer — not taxable
                ...
                sell_price = dest_amount / origin_amount  # FIXED: was origin/dest (inverted)
                ...
                # ratio origin_amount/dest_amount is dimensionless — NOT a EUR price
                sell_price = 0.0
                if price_loader and price_loader.has_asset(origin_asset):
                    loaded = price_loader.get_price(origin_asset, date)
                    if loaded is not None:
